...
def fetch_lesson_facts(cfg: dict, query: str, intent: dict) -> "dict | None":
    ...
    user = (
        ...
        '"synopsis":"课文内容概要（100字内）","background":"写作背景（不限字数，作者在什么境遇/动机下写的）",'
...
def build_few_shot(segments: list[dict], limit: int = 3, maxlen: int = 200) -> str:
    ...
    return (
        "以下是这位老师讲过的真实片段（只参考口吻，不要照抄其中的固定过渡句和结构句）：\n"
        "---\n"
        f"{samples}\n"
        "---\n\n"
    )
...
要求：
# 1. 用第一人称"我"的课堂口吻。
# 2. 只讲本句真正需要讲的重点词（以"重点词"列表为准）。每个重点词 1-2 句讲清即可：读音/本义/语境义/用法按需要取舍，不要机械全讲；非重点词不要额外拆开讲。
# 3. 串讲时不要说任何结构提示，直接写："{sentence_text}。就是说……"，然后开始解释这句在文中起到的作用或好处或学生要体会什么。
#    例如："初，权谓吕蒙曰：'卿今当涂掌事，不可不学！'
#    就是说，当初孙权对吕蒙说：'你现在当权掌事，不能不学习啊！'"
# 4. 利用上一句结尾自然衔接，但不要重复上一句已经讲过的背景、主题、手法或道理；同一个意思不要换说法反复讲。尤其不要用"上一句我们说到……"复述上一句。若上一句结尾是问题或悬念，本句只需顺势进入原文，不要固定使用"他马上给出答案"这类套话。
#    例如上一句：这个“自李唐来”啊，李唐是什么意思呢？
#    紧跟下一句：就是唐朝，因为唐朝的皇帝姓李，所以后人常常称唐朝为“李唐”。
# 5. 篇幅控制：普通短句 250-450 字；长句或重点词超过 4 个时 500-750 字。宁可短一点，也不要重复。
# 6. **重要**：以下是本句所有的重点词，你必须逐个讲解，并且在讲到该词含义时用方括号 [ ] 标记。
#    例如讲解"静"字时写成 [静]。开头引用原文时不要标记。必须标记所有重点词，一个都不能漏。

1. 用第一人称“我”的课堂口吻。
2. 只讲“重点词”列表中的词。每个重点词只解释其在本句中的含义和作用，1-2句即可。不得扩展历史背景、现实举例、做人道理或考试知识。
3. 开头固定格式：
“{sentence_text}。就是说……”
随后直接串讲，不要出现“这一句”“接下来”“我们来看”“这里很有意思”等结构提示。
4. 讲解重点以“本句在文中的作用”为主，而非词语释义。重点词解释总篇幅不超过全文30%。
5. 对描写句，优先分析：
* 写了什么特点；
* 塑造了什么形象；
* 与前后文形成什么关系。
不要主动引申人生哲理。
6. 不重复上一句已经讲过的主题、情感、手法、中心思想，只分析本句新增的信息。
7. 禁止使用：
   “大家想想”“作者为什么这样写”“给我们什么启示”“做人是不是也要这样”“这里很有意思”等课堂填充语。
8. 当篇幅超出限制时，优先删除：
   1.重复解释；
   2.现实联想；
   3.做人启示；
   4.课堂互动语。
9. 必须讲解全部重点词，并在首次解释该词含义时使用方括号标记，例如：[染]、[濯]。不得遗漏。
10. 普通句控制在250-450字；长句或重点词超过4个时控制在500-750字。
...
def _build_context(
    ...
    if prev_narration and sentence_idx > 1:
        ...
        parts.append(
            f"\n上一句你讲完时的最后一段话：\"{tail}\"\n"
            f"请顺着这句话继续，不要复述上一句的内容。"
            f"如果上一句最后留下问题或悬念，本句开头直接进入原文或答案，"
            f"但不要使用固定套话，不要每句都用相同开头。"
        )
