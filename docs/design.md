...
#### 步骤 3.5：获取课文硬事实（讲解类意图触发）
- 在检索同时，额外调一次大模型获取指定课文的**作者、朝代、出处、原文**等权威信息，并把原文**逐句拆开**，每句标注白话翻译和重点字词释义（实词/虚词/活用/古今异义/通假优先），形成"逐句精讲骨架"。
- 输出结构：`{"author":"","dynasty":"","source":"","excerpt":"","sentences":[{"text":"","translation":"","keywords":[{"word":"","note":""}]}],"synopsis":"","keyPoints":[]}`
- 这些事实注入 System Prompt 作为不可违背的硬约束，禁止生成模型自行编造。
- 只在意图为精读讲解/字词分析/背景导入/概括大意时触发。

#### 步骤 4：组装 Prompt
- 模板结构：
  ```
  [System] 你是XXX老师的课堂克隆。风格档案：{A.3 产出的风格档案 JSON}
  课文硬事实：作者/朝代/出处/原文（作为不可违背的已知事实）
  ...
  [输出格式]
  【课文信息】列出作者、朝代、出处（严格引用 System 中硬事实）
  有逐句骨架时 → 【精讲正文】按骨架顺序逐句/逐段讲：引原文 → 逐个解读重点词 → 串讲
  无骨架时     → 【课堂讲解】常规讲解
  ...
  4. **不限字数**；篇幅短则逐句讲，篇幅长则把连贯的几句合成一段逐段讲，但每个重点词都要落到，不概括省略。
  5. 字词读音/释义/出处严格依据骨架，宁可不展开也不编。
  6. 结尾可用提问或互动引导。
  ```
